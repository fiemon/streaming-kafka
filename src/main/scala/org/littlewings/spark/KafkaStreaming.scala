...
//    System.setProperty("twitter4j.oauth.accessTokenSecret", config.get("oauth.accessTokenSecret").toString)
    ...
    // kafka
//    val stream = KafkaUtils.createStream(ssc, "devblog-kafka:2181", "console-consumer", Map("syslog-topic" -> 1)
    val stream = KafkaUtils.createStream(ssc, "devblog-kafka:2181", "0", Map("syslog-topic" -> 1))
    val kafkaInputCount = stream.count()
...
//    val wordAndOnePairRDD = stream.map(word => (ngwordConvert(word)))
...
//    sortedCountAndWordRDD.saveAsTextFiles(config.get("save.file.dir").toString + args(1))
    stream.print()
//    stream.saveAsTextFiles(config.get("save.file.dir").toString + args(1))
